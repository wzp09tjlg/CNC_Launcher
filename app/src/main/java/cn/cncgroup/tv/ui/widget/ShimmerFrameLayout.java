...
public class ShimmerFrameLayout extends FrameLayout
{
	...
	private static final PorterDuffXfermode DST_IN_PORTER_DUFF_XFERMODE = new PorterDuffXfermode(
	...
	public ShimmerFrameLayout(Context context, AttributeSet attrs, int defStyle)
	{
		...
		if (attrs != null)
		{
			...
			try
			{
				...
				{
					setDuration(a.getInt(
					        R.styleable.ShimmerFrameLayout_duration, 0));
				}
				...
				if (a.hasValue(R.styleable.ShimmerFrameLayout_repeat_mode))
				{
					setRepeatMode(a.getInt(
					        R.styleable.ShimmerFrameLayout_repeat_mode, 0));
				}
				...
				{
					mMask.tilt = a.getFloat(
					        R.styleable.ShimmerFrameLayout_tilt, 0);
				}
			} finally
			{
				a.recycle();
	 ...
	 * create the bitmap one more time.
	 *
	 * @param width
	 *            width of the bitmap
	 * @param height
	 ...
	 * modified will be reverted back to their original value. Also, stops the
	 * shimmer animation if it is currently playing.
	 */
	 ...
	 * layout will start animating automatically whenever it is attached to the
	 * current window.
	 *
	 ...
	 * enabled, the layout will start animating automatically whenever it is
	 ...
	 * highlight to move from one end of the layout to the other. The default
	 * value is 1000 ms.
	 *
	 ...
	 * highlight to move from one end of the layout to the other.
	 *
	 * @param duration
	 ...
	 * value is -1, which means the animation will repeat indefinitely.
	 *
	 ...
	 * is 0, the animation stops after reaching the end. If greater than 0, or
	 * -1 (for infinite), the repeat mode is taken into account.
	 *
	 * @param repeatCount
	 *            Number of times the current animation should repeat, or -1 for
	 *            indefinite.
	 */
	 ...
	 * value is 0, which means the animation will repeat immediately, unless it
	 * has ended.
	 *
	 ...
	 *
	 * @param repeatDelay
	 *            Delay after which the animation should repeat, in
	 *            milliseconds.
	 */
	...
	/**
	 * Get what the current animation will do after reaching the end. One of <a
	 * href=
	 * "http://developer.android.com/reference/android/animation/ValueAnimator.html#REVERSE"
	 * >REVERSE</a> or <a href=
	 * "http://developer.android.com/reference/android/animation/ValueAnimator.html#RESTART"
	 * >RESTART</a>
	 *
	...
	/**
	 * Set what the animation should do after reaching the end. One of <a href=
	 * "http://developer.android.com/reference/android/animation/ValueAnimator.html#REVERSE"
	 * >REVERSE</a> or <a href=
	 * "http://developer.android.com/reference/android/animation/ValueAnimator.html#RESTART"
	 * >RESTART</a>
	...
	/**
	 * Get the shape of the current animation's highlight mask. One of
	 * {@link MaskShape#LINEAR} or {@link MaskShape#RADIAL}
	 *
	...
	/**
	 * Set the shape of the animation's highlight mask. One of
	 * {@link MaskShape#LINEAR} or {@link MaskShape#RADIAL}
	 *
	 * @param shape
	 ...
	 * opacity is 0 i.e it is fully transparent. For a linear mask, the distance
	 * is relative to the center towards the edges. For a radial mask, the
	 * distance is relative to the center towards the circumference. So a
	 * dropoff of 0.5 on a linear mask will create a band that is half the size
	 * of the corresponding edge (depending on the {@link MaskAngle}), centered
	 * in the layout.
	 *
	 * @param dropoff
	 */
	 ...
	 * intensity controls the brightness of the highlight; the higher it is, the
	 * greater is the opaque region in the highlight. The default value is 0.
	 *
	 ...
	 * dropping off, so an intensity of 0 would mean that the highlight starts
	 * becoming translucent immediately from the center (the spread is
	 * controlled by 'dropoff').
	 *
	 * @param intensity
	 ...
	 * default is 1.0, meaning that the mask is of the same width as the layout.
	 *
	 ...
	 * default is 1.0, meaning that the mask is of the same height as the
	 * layout.
	 *
	 ...
	 * method is called automatically when the layout is attached to the current
	 * window. Calling this method has no effect if the animation is already
	 * playing.
	 */
	...
	private void drawMasked(Canvas renderCanvas)
	{
		...
		renderCanvas.clipRect(mMaskOffsetX, mMaskOffsetY, mMaskOffsetX
		        + maskBitmap.getWidth(), mMaskOffsetY + maskBitmap.getHeight());
		renderCanvas.drawColor(0, PorterDuff.Mode.CLEAR);
		super.dispatchDraw(renderCanvas);
	...
	// If a mask bitmap was created, it's recycled and set to null so it will be
	...
	// If a working bitmap was created, it's recycled and set to null so it will
		...
		switch (mMask.shape)
		{
			...
			case LINEAR:
			{
				...
				gradient = new LinearGradient(x1, y1, x2, y2,
				        ...
				        mMask.getGradientPositions(), Shader.TileMode.REPEAT);
				break;
			}
			case RADIAL:
			{
				int x = width / 2;
				int y = height / 2;
				gradient = new RadialGradient(x, y, (float) (Math.max(width,
				        height) / Math.sqrt(2)), mMask.getGradientColors(),
				        mMask.getGradientPositions(), Shader.TileMode.REPEAT);
				break;
			}
		}
	...
	// Get the shimmer <a
	// href="http://developer.android.com/reference/android/animation/Animator.html">Animator</a>
	...
	private Animator getShimmerAnimation()
	{
		...
		mAnimator = ValueAnimator.ofFloat(0.0f, 1.0f + (float) mRepeatDelay
		        / mDuration);
		...
		mAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener()
		{
			...
			public void onAnimationUpdate(ValueAnimator animation)
			{
				...
				setMaskOffsetX((int) (mMaskTranslation.fromX * (1 - value) + mMaskTranslation.toX
				        * value));
				setMaskOffsetY((int) (mMaskTranslation.fromY * (1 - value) + mMaskTranslation.toY
				        * value));
			}
		});
		return mAnimator;
	...
	// enum specifying the shape of the highlight mask applied to the contained
	// view
	...
	// struct storing various mask related parameters, which are used to
	// construct the mask bitmap
	private static class Mask
	{
		...
		{
			return fixedHeight > 0 ? fixedHeight
